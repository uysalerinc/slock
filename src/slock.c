...
{/*Function that displays given time on the given screen*/
	...
	xpos=DisplayWidth(dpy, screen)/2 - 20*textsize/3;
	ypos=DisplayHeight(dpy, screen)/2;
	sprintf(tm," %02d/%02d/%d %s %02d:%02d",time.tm_mday,time.tm_mon + 1,time.tm_year+1900, clc_icon,time.tm_hour,time.tm_min);
...
usage(void)
{
	die("usage: slock [-v] [-f] [-m message] [-b background_image_path] [cmd [arg ...]]\n");
}
